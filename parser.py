...
_PROMPT_TEMPLATE = (
    ...
    "\n"
    "【描述质量要求（重要：描述将用于 AI 命令路由匹配）】\n"
    "- 插件描述要具体：说明插件提供什么功能、解决什么问题，不要写\"这是一个xxx插件\"\n"
    "- 命令描述要包含「动作+对象+范围」：如\"查询指定城市的实时天气信息\"而非\"查看天气\"\n"
    "- 描述要能区分相似命令：如果插件有多个命令，每个命令的描述必须体现其独特功能\n"
    "- 如果某命令有功能限制（如\"不支持xx\"、\"仅限xx\"），明确写在描述中\n"
    "- 避免\"无描述\"或空描述，即使 README 信息有限也要根据命令名合理推断\n"
    "- 不同命令的描述不能雷同，否则 AI 无法区分该调用哪个\n"
    "\n"
    "【参数识别规则】\n"
    ...
    "- args_description 必须包含每个参数的含义、是否必填、可选值或默认值\n"
    "\n"
    "【注意事项】\n"
